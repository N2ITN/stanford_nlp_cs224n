...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        def left_arc():
            t = (self.stack[-1], self.stack.pop(-2))
            # print('t_left', t)
            self.dependencies.append(t)

        def right_arc():
            t = (self.stack[-2], self.stack.pop(-1))
            # print('t_right', t)
            self.dependencies.append(t)

        switch = {'S': shift, 'LA': left_arc, 'RA': right_arc}
...
def minibatch_parse(sentences, model, batch_size):
    ...
    #     preds = model.predict(mini)

    #     print('preds', preds)
    #     unfinished_parses = []
    #     for p, m in zip(preds, mini):
    #         res = m.parse([p])
    #         print(m.sentence,p,res)

    #         unfinished_parses = [p for p in unfinished_parses if len(p.buffer) == 0 & len(p.stack) == 1]
    ...
    while len(unfinished_parses) > 0:
        ...
        for parses, transition in zip(parsers, batch_transitions):
            parses.parse([transition])
