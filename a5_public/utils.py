...
def pad_sents_char(sents, char_pad_token):
    ...
    def pad_word(w):
        return w[:max_word_length] + [char_pad_token]*(max_word_length-len(w))

    def pad_sents(s):
        pad = ([char_pad_token]*max_word_length)
        delta  = max_sentence_length - len(s)
        if delta > 0:
            s += [pad] * delta 
        return s 

    sents_padded=[pad_sents([pad_word(w) for w in s]) for s in sents]
