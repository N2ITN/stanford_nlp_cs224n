...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        return [[[self.start_of_word]+[self.char2id.get(c, 3) for c in w]+[self.end_of_word] for w in s] for s in sents]
        # END YOUR CODE
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        # previous parts
        res = torch.tensor(pad_sents_char(self.words2charindices(
            sents), char_pad_token=self.char2id['<pad>']), device=device)
